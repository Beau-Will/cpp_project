...
void solve(){
  ...
  while(std::cin >> a >> b >> c){
    ...
    std::vector<int> inita(c * 5 + b * 2 + a + 2);
    for(int i = 0; i <= a; i++){
      inita[i] = 1;
    }
    
    std::vector<int> initb(c * 5 + b * 2 + a + 2);
    for(int i = 0; i <= b * 2; i += 2){
      initb[i] = 1;
    }

    std::vector<int> initc(c * 5 + b * 2 + a + 2);
    for(int i = 0; i <= c * 5; i += 5){
      initc[i] = 1;
    }

    OGF ans = OGF(inita) * OGF(initb) * OGF(initc);

    for(int i = 0; i < c * 5 + b * 2 + a + 2; i++){
      if(ans.coeffs[i] == 0){
        std::cout << i << "\n";
        break;
      }
