...
Info operator+(const Info &info1,const Info &info2){
	return {std::__gcd(info1.gcd,info2.gcd)};
}
...
void solve(){
	...
	// std::vector<Info> diff(n);
	// for(int i = 0;i<n;++i){
		// std::cin >> a[i];
	// }
	// for(int i = 1;i<n;++i){
		// diff[i].gcd = std::abs(a[i]-a[i-1]);
	// }
	// SegmentTree<Info> seg(diff);
	// while(m--){
		// int l,r;std::cin >> l >> r;
		// std::cout << seg.rangeQuery(l,r).gcd << " \n"[m==0];
	// }
